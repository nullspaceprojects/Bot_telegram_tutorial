...
const String TELEGRAM_BOT_TOKEN = "123456789:ABCDEFGKIJ6547DCVDV";
...
void loop() {
  ...
  if(looptimer.getET()>1000)
  {
    ...
    else if( //command to write on OLED: /#text-to-display if we are writing on a group chat where the bot is a member.
            //                            #text-to-display (without the "/" if we are writing to the bot through the private chat)
          (received_cmd.indexOf("/")==0 && received_cmd.indexOf("#")==1 && !isPrivateChat) //group chat
      ||  (received_cmd.indexOf("#")==0 && isPrivateChat) ) //private chat
    {
      String oled_text;
      if(isPrivateChat)
      {
